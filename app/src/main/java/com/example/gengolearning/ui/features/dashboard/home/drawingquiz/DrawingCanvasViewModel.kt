package com.example.gengolearning.ui.features.dashboard.home.drawingquiz
    ...
    init {
            if (quizzes.isNotEmpty()) {
                ...
                repository.words.onEach { list ->
                    if (list.isNotEmpty()) {
                        val filteredWords = list.filter { it.isOnHomePage == true }
                        if (filteredWords.isNotEmpty()) {
                            currentIndex = 0
                            currentWord.value = filteredWords[0]
                            _currentList.value = filteredWords
