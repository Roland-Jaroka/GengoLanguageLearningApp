package com.example.mylanguagelearningapp.ui.theme

import androidx.compose.ui.graphics.Color

val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)

val Blue = Color(0xFF1E88E5)
val SoftGray = Color(0xFFE0E0E0)
val NeutralGray = Color(0xFF9E9E9E)
val BlueGray = Color(0xFF3949AB)
val DeepBlue = Color(0xFF1565C0)
val LightBlue = Color(0xFF90CAF9)
val White = Color(0xFFFFFFFF)
val BgBlue= Color(0xFF1F69A1)
val Red= Color(0xFFFA0505)

val LeafGreen = Color(0xFF5E8B4A)
val TextDarkBlue = Color(0xFF2C3E50)
val ButtonBackground = Color(0xFFEFE3C6)
val ButtonBorder = Color(0xFF4A6B3C)
val NavBarBackground = Color(0xFFEAD9B0)
val NavIconGreen = Color(0xFF3C5F3D)
val PandaBlack = Color(0xFF2B2B2B)
val PandaWhite = Color(0xFFFDFDFD)
val AccentRed = Color(0xFFD32F2F)
